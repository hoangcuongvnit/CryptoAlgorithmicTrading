...
public sealed class OrderStatsRepository
{
    ...
    /// </summary>
    public async Task<decimal> GetDailyNetPnlAsync(CancellationToken ct = default)
    {
        const string sql = """
    ...
    public async Task<WinRateStats> GetWinRateAsync(
        string? symbol, int lookbackDays, CancellationToken ct = default)
    {
        const string sql = """
            SELECT
        ...
        return await conn.QuerySingleOrDefaultAsync<WinRateStats>(
                        new CommandDefinition(sql, new { symbol, lookbackDays }, cancellationToken: ct))
