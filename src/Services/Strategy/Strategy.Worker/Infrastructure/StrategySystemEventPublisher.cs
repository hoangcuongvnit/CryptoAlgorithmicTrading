...
public sealed class StrategySystemEventPublisher
{
    private readonly IConnectionMultiplexer _redis;
    private readonly ILogger<StrategySystemEventPublisher> _logger;

    public StrategySystemEventPublisher(IConnectionMultiplexer redis, ILogger<StrategySystemEventPublisher> logger)
    {
        _redis = redis;
        _logger = logger;
    }
    ...
    /// <summary>
    /// Publishes a system event to system:events Redis channel.
    ...
    public async Task PublishAsync(SystemEvent evt, CancellationToken ct = default)
    {
        ...
        {
            _logger.LogError(ex, "Failed to publish system event {Type} from Strategy", evt.Type);
        }
    }
}
